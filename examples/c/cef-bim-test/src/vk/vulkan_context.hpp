...
// CUDA replaced with stand-in UUID type — cef-bim-test doesn't have a
// CUDA producer, but the device-selection API still wants a UUID. An
// all-zero UUID means "first available physical device meeting feature
// requirements". Same wire format as VkPhysicalDeviceIDProperties.deviceUUID.
...
public:
    ...
    auto physical_device() const -> VkPhysicalDevice { return _physical_device; }
    ...
    auto descriptor_set_layout() const -> VkDescriptorSetLayout { return _descriptor_set_layout; }

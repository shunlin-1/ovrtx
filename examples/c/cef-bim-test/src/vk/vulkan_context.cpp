...
auto VulkanContext::_select_physical_device(const CUuuid& cuda_uuid, uint32_t requested_sampler_capacity) -> void {
    ...
    for (const auto& device : devices) {
        ...
        // All-zero UUID is the cef-bim-test sentinel for "any device" —
        // we don't have a CUDA producer to anchor to in this build.
        const auto* req = reinterpret_cast<const uint8_t*>(cuda_uuid.bytes);
        bool req_is_zero = true;
        for (int i = 0; i < 16; ++i) { if (req[i] != 0) { req_is_zero = false; break; } }
        if (req_is_zero || uuid_matches(id_props.deviceUUID, req)) {
            printf("  -> Selected device%s\n",
                   req_is_zero ? " (any UUID accepted)" : " (UUID matched)");
    ...
    throw std::runtime_error(
        "No Vulkan physical device available (or UUID match failed)");
}
...
auto VulkanContext::_create_descriptor_set_layout() -> void {
    ...
    push_constant_range.size = sizeof(uint32_t) * 2;  // scene_index + ui_index
